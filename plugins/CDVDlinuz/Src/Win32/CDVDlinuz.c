...
s32 CALLBACK CDVDopen()
{
	...
	retval = DeviceOpen();
	if (retval == 0)
		DeviceTrayStatus(); // ENDIF- Did we open the device? Poll for media.
	return (retval);
} // END CDVDopen()
...
s32 CALLBACK CDVDreadTrack(u32 lsn, int mode)
{
	...
	if (userbuffer < BUFFERMAX) {
		if ((bufferlist[userbuffer].lsn == lsn) &&
			...
			return (0); // ENDIF- And it's the right one?
	} // ENDIF- Are we already pointing at the buffer?
	userbuffer = FindListBuffer(lsn);
	if (userbuffer < BUFFERMAX) {
		if ((bufferlist[userbuffer].lsn == lsn) &&
		    (bufferlist[userbuffer].mode == mode))
			return (0); // ENDIF- And it was the right one?
	...
	if (bufferlist[replacebuffer].upsort != 0xffff)
		RemoveListBuffer(replacebuffer); // ENDIF- Reference already in place? Remove it.
	...
	if (retval != 0) {
		...
		if ((disctype != CDVD_TYPE_PS2DVD) && (disctype != CDVD_TYPE_DVDV)) {
			if (mode == CDVD_MODE_2352)
				CDreadSubQ(lsn, &bufferlist[replacebuffer].subq); // ENDIF- Read subq as well?
