...
void InitBuffer()
{
	...
	for (i = 0; i < BUFFERMAX; i++)
		buffersortempty[i].sortptr = i; // NEXT i- Saying all buffersort[] entries are open.
	...
	for (i = 0; i < BUFFERMAX; i++)
		bufferlist[i].upsort = 0xffff; // NEXT i- Clearing out the buffer pointers
	userbuffer = 0xffff;
...
u16 AllocSortBuffer()
{
	...
	if (buffersortemptyend == buffersortemptystart)
		PrintLog("Completely out of Sort Buffers to allocate now!"); // ENDIF- Out of Sort Buffers? Say so!
		...
		if (h == BUFFERMAX) {
			...
			for (j = 0; j < 256; j++) {
				...
				if (buffersort[i].lower[j].isdata < 2)
					printf("%u:%u", j, buffersort[i].lower[j].ptr); // ENDIF- We have active data? Print it.
