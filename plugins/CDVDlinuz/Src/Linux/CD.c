...
s32 CDgetTD(u8 newtrack, cdvdTD *cdvdtd)
{
	...
	if (j == 0)
		...
		k = j * 10 + 37; // ENDIF- Where to start hunting for this number?
	if (cdvdtd != NULL) {
...
s32 CALLBACK CDgetDiskType(s32 ioctldisktype)
{
	...
	if ((s32result != 0) || (errno != 0))
		return (-1); // ENDIF- Cannot read the CD's ISO9660 volume sector? Abort
	...
	switch (ioctldisktype) {
		...
		case CDS_MIXED:
			...
			if (*(playstationcdname + i) == 0) {
				...
				tempdisctype = CDVD_TYPE_UNKNOWN; // ENDIF- Did we find the Playstation name?
			break;
		default:
			return (-1);
