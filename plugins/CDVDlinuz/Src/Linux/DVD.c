...
void HexDump(u8 *strptr, u8 count)
{
	...
	for (i = 0; i < count; i++)
		hexlinepos += sprintf(&hexline[hexlinepos], "%.2x ", (*(strptr + i)) * 1); // NEXT i- printing each new Hex number
	for (i = 0; i < count; i++) {
...
s32 DVDreadCopyright()
{
	...
	for (i = 0; i <= dvdphysical.physical.layer_num; i++) {
		...
		if (s32result == 0)
			...
			dvdcopyright[i].type = 0xFF; // ENDIF-
...
s32 DVDreadBCA()
{
	...
	for (i = 0; i < 188 - 15; i += 16)
		HexDump(dvdbca.bca.value + i, 16); // NEXT i- dumping whole key data
#endif /* VERBOSE_DISC_INFO */
...
s32 DVDreadManufact()
{
	...
	for (i = 0; i <= dvdphysical.physical.layer_num; i++) {
		...
		if ((s32result != 0) || (errno != 0))
			...
			successflag = 1; // ENDIF- Did we fail to read in some manufacturer data?
	...
	for (i = 0; i <= dvdphysical.physical.layer_num; i++) {
		if (dvdmanufact[i].type != 0xFF) {
			...
			for (j = 0; j < 128 - 15; j += 16)
				HexDump(dvdmanufact[i].manufact.value + j, 16); // NEXT j- dumping whole key data
...
s32 DVDgetDiskType(s32 ioctldisktype)
{
	...
	if ((ioctldisktype != CDS_DATA_1) && (ioctldisktype != CDS_MIXED))
		return (-1); // ENDIF- Not a data disc we know of? Abort then
	s32result = DVDreadPhysical();
	if ((s32result != 0) || (errno != 0))
		return (-1); // ENDIF- Error reading the DVD physical structure? Not a DVD after all.
	...
	if (s32result != 0)
		...
		while ((*(playstationname + i) != 0) &&
			...
			i++; // ENDWHILE- Checking each letter of PLAYSTATION name for a match
