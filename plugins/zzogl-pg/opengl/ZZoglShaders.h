...
struct FRAGMENTSHADER {
	...
	void set_context(uint new_context)
	{
		context = new_context * NOCONTEXT;
	}
};
#endif
...
static __forceinline FRAGMENTSHADER* curr_ppsCRTC()
{
	return &ppsCRTC[interlace_mode];
}
//static __forceinline FRAGMENTSHADER* curr_ppsCRTC24() { return &ppsCRTC24[interlace_mode]; }
static __forceinline FRAGMENTSHADER* curr_ppsCRTCTarg()
{
	return &ppsCRTCTarg[interlace_mode];
}
...
static __forceinline FRAGMENTSHADER* curr_pps(CRTC_TYPE render_type)
{
	switch (render_type) {
		...
		case CRTC_RENDER_TARG:
			return curr_ppsCRTCTarg();
		default:
			return NULL;
	}
}
...
inline bool ZZshExistProgram(ZZshShaderLink prog)
{
	return (prog.link != NULL);        // This is used for pvs mainly. No NULL means that we do LOAD_VS
}
#endif
