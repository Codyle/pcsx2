...
string dumpAVICOMPRESSOPTIONS(AVICOMPRESSOPTIONS opts)
{
	...
	returnValue += "DWORD  fccType = streamtype(";
	returnValue += getFOURCCVAsString(opts.fccType);
	returnValue += ")\n";
	returnValue += "DWORD  fccHandler = ";
	returnValue += getFOURCCVAsString(opts.fccHandler);
	returnValue += "\n";
	...
	if ((opts.dwFlags & AVICOMPRESSF_DATARATE) == AVICOMPRESSF_DATARATE)
		strcpy(tmp, "DWORD  fccType = AVICOMPRESSF_DATARATE\n");
	else if ((opts.dwFlags & AVICOMPRESSF_INTERLEAVE) == AVICOMPRESSF_INTERLEAVE)
		strcpy(tmp, "DWORD  fccType = AVICOMPRESSF_INTERLEAVE\n");
	else if ((opts.dwFlags & AVICOMPRESSF_KEYFRAMES) == AVICOMPRESSF_KEYFRAMES)
		strcpy(tmp, "DWORD  fccType = AVICOMPRESSF_KEYFRAMES\n");
	else if ((opts.dwFlags & AVICOMPRESSF_VALID) == AVICOMPRESSF_VALID)
		strcpy(tmp, "DWORD  fccType = AVICOMPRESSF_VALID\n");
	else
		_snprintf(tmp, 255, "DWORD  dwFlags = Unknown(%d)\n", opts.dwFlags);
	returnValue += tmp;
