			...
			for (iL = 0; iL < 8; iL++) {
				...
				if (sd.SRB_Status == SS_COMP &&
					...
					if (s.SRB_Status == SS_COMP) {
						int i;
						for (i = 8 ; i < 15; i++) if (szBuf[i] == ' ') {
								szBuf[i] = 0;
								break;
							}
						szBuf[15] = 0;
						for (i = 16; i < 32; i++) if (szBuf[i] == ' ') {
								szBuf[i] = 0;
								break;
							}
						szBuf[31] = 0;
						for (i = 32; i < 37; i++) if (szBuf[i] == ' ') {
								szBuf[i] = 0;
								break;
							}
						szBuf[36] = 0;
	...
	for (i = 0; i < MAXMODES; i++) {                      // loop avail read modes
		...
		for (j = 0; j < 3; j++) {                           // try it 3 times
			...
			for (k = 0, iCnt = 0; k < (int)f->dwBufLen; k += 4, p += 4) { // now check the returned data
				...
				if (iCnt >= 8) {
					dwStatus = SS_ERR;        // -> if we have found many AA's, the reading was bad
					break;
				}
			}
