...
void CreateREADBufs(void)
{
	switch (iUseCaching) {
		case 4:
			iUseDataCache  = 2; // use a special data cache on threadex reading
			pReadTrackFunc = DoReadThreadEx;
			pGetPtrFunc    = GetREADThreadExPtr;
			break;
		case 3:
			pReadTrackFunc = DoReadThread;
			pGetPtrFunc    = GetREADThreadPtr;
			break;
		case 2:
			pReadTrackFunc = DoReadAsync;
			pGetPtrFunc    = NULL;
			break;
		default:
			pReadTrackFunc = DoRead;
			pGetPtrFunc    = NULL;
			break;
	}
...
BOOL DoReadAsync(unsigned long addr)
{
	...
	if (lLastAsyncAddr != 0xFFFFFFFF &&
		...
		if (iBufSel == 0) iBufSel = 1;
		else iBufSel = 0;         // toggle to new num
	...
	iTryAsync++;
	if (iTryAsync > 1)  {
		iTryAsync = 2;        // prefetches seems to be useless right now, so turn them off until next real read
		return TRUE;
	}
...
BOOL DoReadThread(unsigned long addr)
{
	...
	if (iUseDataCache && CheckDataCache(addr)) {          // data cache used and data is in cache? set read ptr, if yes
		bDataCacheHit = TRUE;        // -> and raise 'hit' flag, so we don't need to do anything in 'getbuffer'
		return TRUE;
	}
...
BOOL DoReadThreadEx(unsigned long addr)
{
	...
	for (k = 0; k < MAXQSIZE; k++) {                      // loop max of prefetch blocks
		...
		if (i != MAXDATACACHE) {                            // found in data cache?
			addr = addr + MAXCACHEBLOCK + 1;        // -> do nothing with this addr, we have its content
			continue;
	...
	for (k = 0; k < MAXQFETCH; k++) {                     // loop max of prefetch blocks
		...
		if (i != MAXQSIZE) {                                // found in q?
			addr = addr + MAXCACHEBLOCK + 1;        // -> do nothing with this addr, we will have its content soon
			continue;
		}
...
void AddToDataCache(unsigned long addr, unsigned char * pB)
{
	...
	if (iPos == iQLockPos) {                              // special thread mode lock?
		iPos++;        // -> don't use that pos, use next one
		if (iPos >= MAXDATACACHE) iPos = 0;
	}
