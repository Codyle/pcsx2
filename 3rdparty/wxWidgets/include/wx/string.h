...
inline bool IsEmpty(const char *p)
{
	return (!p || !*p);
}
...
#pragma warning (default:4714)
	...
	// string += string
	wxStringBase &operator+=(const wxStringBase &s)
	{
		return append(s);
	}
	// string += C string
...
public:
	...
	const wxChar* wc_str(const wxMBConv &WXUNUSED(conv)) const
	{
		return c_str();
	}
...
#else // !wxMBFILES
	const wxChar* fn_str() const
	{
		return c_str();
	}
#endif // wxMBFILES/!wxMBFILES
#else // ANSI
	const wxChar* mb_str() const
	{
		return c_str();
	}

	// for compatibility with wxUSE_UNICODE version
	const wxChar* mb_str(const wxMBConv &WXUNUSED(conv)) const
	{
		return c_str();
	}
	...
	const wxCharBuffer fn_str() const
	{
		return wxConvFile.cWC2WX(wc_str(wxConvLocal));
	}
#else
	const wxChar* fn_str() const
	{
	...
	{
		if (psz) wxStringBase::operator=(psz);
		else Clear();
		return *this;
	}
#else
	...
	wxString Before(wxChar ch) const
	{
		return BeforeLast(ch);
	}
	wxString After(wxChar ch) const
	{
	...
	bool Alloc(size_t nLen)
	{
		reserve(nLen); /*return capacity() >= nLen;*/ return true;
	}
	...
	// use Find()
	int First(const wxChar ch) const
	{
		return Find(ch);
	}
	...
	int Last(const wxChar ch) const
	{
		return Find(ch, true);
	}
...
public:
	...
	~wxStringBuffer()
	{
		m_str.assign(m_buf.data(), wxStrlen(m_buf.data()));
	}
...
// be always available)
inline bool operator==(wxChar c, const wxString &s)
{
	return s.IsSameAs(c);
}
inline bool operator==(const wxString &s, wxChar c)
{
	return s.IsSameAs(c);
}
inline bool operator!=(wxChar c, const wxString &s)
