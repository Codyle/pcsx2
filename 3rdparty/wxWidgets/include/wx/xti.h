...
public :
	...
	{
		if (m_toString)(*m_toString)(data , result) ;
	...
	{
		if (m_fromString)(*m_fromString)(data , result) ;
		else wxLogError(_("String conversions not supported")) ;
	}
...
public :
	...
	{
		if (m_toLong)(*m_toLong)(data , result) ;
	...
	{
		if (m_fromLong)(*m_fromLong)(data , result) ;
		else wxLogError(_("Long Conversions not supported")) ;
	}
...
public :
	...
	wxCollectionTypeInfo(const char *elementName , converterToString_t to , converterFromString_t from  , const char *name) :
		wxTypeInfo(wxT_COLLECTION , to , from , name)
	{
		m_elementTypeName = wxString::FromAscii(elementName) ;
		m_elementType = NULL ;
	}
#endif
...
public:
	...
	// return a heap allocated duplicate
	virtual wxxVariantData* Clone() const
	{
		return new wxxVariantDataT<T>(Get()) ;
	}
...
public :
	...
	{
		if (d.m_data) m_data = d.m_data->Clone() ;
		else m_data = NULL ;
		m_name = d.m_name ;
	}
...
{
public :
	wxCollectionGetter(const wxString name)
	{
		m_name = name ;
	}
...
public :
	...
	{
		if (m_setter) m_setter->Set(object , value) ;
	...
	{
		if (m_getter) m_getter->Get(object , result) ;
	...
	{
		if (m_adder) m_adder->Add(object , value) ;
	...
	{
		if (m_collectionGetter) m_collectionGetter->Get(obj , result) ;
...
public :
	...
	wxxVariant          GetDefaultValue() const
	{
