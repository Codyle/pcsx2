...
public:
	...
	{
		return m_numCols;        // number of "defined" columns for this wxDbTable instance
	}
...
#if wxODBC_BACKWARD_COMPATABILITY
	void            SetFromClause(const char *From)
	{
		from = (char *)From;
	}
	void            SetOrderByClause(const char *OrderBy)
	{
		orderBy = (char *)OrderBy;
	}
	void            SetWhereClause(const char *Where)
	{
		where = (char *)Where;
	}
#else
	...
	{
		return GetNewCursor(setCursor, bindColumns);
	}
#endif
	...
	bool            SetNull(int colNumber, bool set = true)
	{
		return (SetNull(colNumber, set));
	}
	bool            SetNull(const char *colName, bool set = true)
	{
		return (SetNull(colName, set));
	}
#endif
#ifdef __WXDEBUG__
	ULONG           GetTableID()
...
#if wxODBC_BACKWARD_COMPATABILITY
	virtual     wxVariant    GetCol(const int colNumber) const
	{
		return GetColumn(colNumber);
	};
	virtual     void         SetCol(const int colNumber, const wxVariant value)
	{
		return SetColumn(colNumber, value);
	};
#endif
