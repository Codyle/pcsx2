...
struct Gif_Path {
	...
	bool isMTVU()
	{
		return !idx && THREAD_VU1;
	}
	s32 getReadAmount()
	{
		return AtomicRead(readAmount) + gsPack.readAmount;
	}
	bool hasDataRemaining()
	{
		return curOffset < curSize;
	}
	bool isDone()
	{
		return isMTVU() ? !mtvu.fakePackets
...
struct Gif_Unit {
	...
	u32 TransferGSPacketData(GIF_TRANSFER_TYPE tranType, u8* pMem, u32 size, bool aligned = false)
	{
		...
		if (tranType == GIF_TRANS_DMA) {
			if (!CanDoPath3())   {
				if (!Path3Masked()) stat.P3Q = 1;        // DMA Stall
				return 0;
			}
		...
		if (tranType == GIF_TRANS_XGKICK) {
			if (!CanDoPath1())
				stat.P1Q = 1;  // We always buffer path1 packets
		}
		if (tranType == GIF_TRANS_DIRECT) {
			if (!CanDoPath2())   {
				stat.P2Q = 1;        // Direct Stall
				return 0;
			}
		}
		if (tranType == GIF_TRANS_DIRECTHL) {
			if (!CanDoPath2HL()) {
				stat.P2Q = 1;        // DirectHL Stall
				return 0;
			}
		}
	...
	int Execute(bool isPath3, bool isResume)
	{
		...
		for (;;) {
			...
			elif(!gsSIGNAL.queued && !gifPath[1].isDone()) {
				stat.APATH = 2;
				stat.P2Q = 0;
				curPath = 1;
			}
	...
	bool CanDoPath1()
	{
		return (stat.APATH == 0 || stat.APATH == 1
	...
	bool CanDoPath2()
	{
		return (stat.APATH == 0 || stat.APATH == 2
		        || (stat.APATH == 3 && CanDoP3Slice()))
		       && (CanDoGif() == 1);
	}
	// DirectHL
	...
	bool CanDoPath3()
	{
		return ((stat.APATH == 0 && !Path3Masked())
		        ||	  stat.APATH == 3)
		       && (CanDoGif() == 1);
	}
	...
	bool CanDoP3Slice()
	{
		return stat.IMT == 1 && gifPath[GIF_PATH_3].state == GIF_PATH_IMAGE;
	}
	bool CanDoGif()
	{
		return stat.PSE == 0 && (CHECK_GIFREVERSEHACK ? 1 : stat.DIR == 0) && gsSIGNAL.queued == 0;
	}
	//Mask stops the next packet which hasnt started from transferring
	bool Path3Masked()
	{
		return ((stat.M3R || stat.M3P) && (gifPath[GIF_PATH_3].state == GIF_PATH_IDLE || gifPath[GIF_PATH_3].state == GIF_PATH_WAIT));
	}
