...
public:
	...
	explicit wxDirName(const char* src)
	{
		Assign(fromUTF8(src));
	}
	explicit wxDirName(const wxString &src)
	{
		Assign(src);
	}
	...
	{
		return GetPath().ToUTF8();
	}
	wxCharBuffer ToAscii() const
	{
		return GetPath().ToAscii();
	}
	...
	bool IsOk() const
	{
		return wxFileName::IsOk();
	}
	bool IsRelative() const
	{
		return wxFileName::IsRelative();
	}
	bool IsAbsolute() const
	{
		return wxFileName::IsAbsolute();
	}
	...
	void RemoveLast()
	{
		wxFileName::RemoveDir(GetCount() - 1);
	}
	...
	wxFileName operator+(const wxFileName &right) const
	{
		return Combine(right);
	}
	wxDirName operator+(const wxDirName &right)  const
	{
		return Combine(right);
	}
	wxFileName operator+(const wxString &right)  const
	{
		return Combine(wxFileName(right));
	}
	wxFileName operator+(const char* right)  const
	{
		return Combine(wxFileName(fromUTF8(right)));
	}

	bool operator==(const wxDirName &filename) const
	{
	...
	{
		return SameAs(wxDirName(filename));
	}
	bool operator!=(const wxFileName &filename) const
	{
		return !SameAs(wxDirName(filename));
